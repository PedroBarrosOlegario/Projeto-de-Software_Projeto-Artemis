...
function openModal(item){
  document.getElementById('unidadeNome').value = item? (item['unidadeNome' ] || item['Nome'] || '') : '';
...
async function save(){
  try{
    ...
    if(editingId) await apiFetch('/unidades/'+editingId, { method:'PUT', body: JSON.stringify(body) });
    else await apiFetch('/unidades', { method:'POST', body: JSON.stringify(body) });
    hideModal('modal'); loadList(); toast('Salvo');
  }catch(e){ toast('Erro'); }
}
async function loadList(){
  try{ const list = await apiFetch('/unidades');
...
async function editItem(id){ const it = await apiFetch('/unidades/'+id); openModal(it); }
async function delItem(id){ if(!confirm('Confirmar?')) return; await apiFetch('/unidades/'+id, { method:'DELETE' }); loadList(); }
document.getElementById('save_unidades')?.addEventListener('click', save);