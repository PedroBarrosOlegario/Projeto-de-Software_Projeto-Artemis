...
function openModal(item){
  document.getElementById('cursoNome').value = item? (item['cursoNome' ] || item['Nome'] || '') : '';
...
async function save(){
  try{
    ...
    if(editingId) await apiFetch('/cursos/'+editingId, { method:'PUT', body: JSON.stringify(body) });
    else await apiFetch('/cursos', { method:'POST', body: JSON.stringify(body) });
    hideModal('modal'); loadList(); toast('Salvo');
  }catch(e){ toast('Erro'); }
}
async function loadList(){
  try{ const list = await apiFetch('/cursos');
...
async function editItem(id){ const it = await apiFetch('/cursos/'+id); openModal(it); }
async function delItem(id){ if(!confirm('Confirmar?')) return; await apiFetch('/cursos/'+id, { method:'DELETE' }); loadList(); }
document.getElementById('save_cursos')?.addEventListener('click', save);