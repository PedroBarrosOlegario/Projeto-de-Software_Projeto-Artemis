...
function openModal(item){
  document.getElementById('deptNome').value = item? (item['deptNome' ] || item['Nome'] || '') : '';
  editingId = item? item.id : null; showModal('modal');
}
async function save(){
  try{
    const body = {
      'deptNome': document.getElementById('deptNome').value,
    };
    if(editingId) await apiFetch('/departamentos/'+editingId, { method:'PUT', body: JSON.stringify(body) });
    else await apiFetch('/departamentos', { method:'POST', body: JSON.stringify(body) });
    hideModal('modal'); loadList(); toast('Salvo');
  }catch(e){ toast('Erro'); }
}
async function loadList(){
  try{ const list = await apiFetch('/departamentos');
...
async function editItem(id){ const it = await apiFetch('/departamentos/'+id); openModal(it); }
async function delItem(id){ if(!confirm('Confirmar?')) return; await apiFetch('/departamentos/'+id, { method:'DELETE' }); loadList(); }
document.getElementById('save_departamentos')?.addEventListener('click', save);